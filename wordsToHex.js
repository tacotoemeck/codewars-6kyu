...
function wordsToHex(words) {
    return words
        .split(' ')
        .map(word => '#' + word.substring(0, 3).split('').map(char => char.charCodeAt(0).toString(16)).join(''))
        .map(str => (str.length == 5) ? str + '00' : (str.length == 3) ? str + '0000' : (str.length == 1) ? str + '000000' : str)
}